 ...
 */
public class EncryptedContentEncoder {
    ...
    public EncryptedContentEncoder() {
        this(new HashMap<String, KeyPair>(), new HashMap<String, String>());
    }

    public EncryptedContentEncoder(Map<String, KeyPair> keys, Map<String, String> labels) {
        this.keys = keys;
        this.labels = labels;
    }
    ...
    public byte[] encrypt(byte[] plaintext, byte[] salt, byte[] privateKey, String keyid, ECPublicKey dh, byte[] authSecret, Encoding version) throws GeneralSecurityException {
        ...
        if (version == Encoding.AES128GCM) {
            ...
            //log("ciphertext", Utils.concat(header, Utils.concat(encrypted)));
...
//        if ("1".equals(System.getenv("ECE_KEY//log"))) {
